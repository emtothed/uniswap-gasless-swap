...
export async function getSignatureForBatchPermitTransfer(
    signer: ethers.Wallet,
    tokenAddress: string,
    spender: string,
    feeAmountInSmallestUnit: string,
    swapAmountInSmallestUnit: string
) {
        ...
        nonce = ethers.BigNumber.from(
            ethers.utils.keccak256(
                ethers.utils.defaultAbiCoder.encode(
                    ["uint256", "address"],
                    [Math.floor(Date.now() / 1000), signer.address]
                )
            )
        ).toString();

        const { isFree } = await checkNonce(signer.address, nonce);
        isNonceFree = isFree;
        nonceAttempts++;
        if (nonceAttempts > 50) {
    ...
    const permit: PermitBatchTransferFrom = {
        permitted: [
            {
                token: tokenAddress,
                amount: feeAmountInSmallestUnit,
            },
            {
                token: tokenAddress,
                amount: swapAmountInSmallestUnit,
            },
        ] as TokenPermissions[],
        nonce: nonce,
        deadline: deadline,
...
export async function submitBatchPermitTransferFrom(
    tokenAddress: string,
    feeAmountInSmallestUnit: string,
    swapAmountInSmallestUnit: string
) {
    ...
    const { permit, signature } = await getSignatureForBatchPermitTransfer(
        ...
        tokenAddress,
        server.address,
        feeAmountInSmallestUnit,
        swapAmountInSmallestUnit
    );

    // 2) Build SignatureTransferDetails
    const transferDetails = [
        {
            to: server.address, // or any address you want to send the tokens to
            requestedAmount: feeAmountInSmallestUnit.toString(),
        },
        {
            to: "0x1BeA861967DfaebEA0bCce7c144c98227Db379A1", // or any address you want to send the tokens to
            requestedAmount: swapAmountInSmallestUnit.toString(),
        },
    ];

    // 3) Connect to Permit2 contract with the account that will send transactions
    ...
    ](permit, transferDetails, alice.address, signature);
...
//     // await getBalance("fee rec", "0x1BeA861967DfaebEA0bCce7c144c98227Db379A1");
//     // await getBalance("Alice", "0x910F4a7143Ef72766dDe042FE067218c1dE74b2d");
//     await submitBatchPermitTransferFrom(
